...
export default class SlideIn extends Component {
	constructor(props) {
		super(props);
		const { percentageInViewport=0.2, isSegmentInViewport=false, isAnimationSlideIn=true, animationDelay="delay-020" } = props;
		this.state = {
   ...
   */
	...
	render() {
		const { isSegmentInViewport, isAnimationSlideIn, animationDelay } = this.state;

		let defaultClasses = `${isAnimationSlideIn ? "slide-in-bottom" : "animation-ease-in"} ${animationDelay} ${isSegmentInViewport ? "animation-running" : "animation-paused"}`;
