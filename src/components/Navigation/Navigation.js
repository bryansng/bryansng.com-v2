            ...
            <div className="w-100 flex flex-row flex-nowrap justify-between items-center">
              ...
              <div className="z-9999">
                <SlideIn
                  isSegmentInViewport={true}
                  isAnimationSlideIn={false}
                  animationDelayClass="delay-075"
                >
                  <div
                    ref={this.burger}
                    className="dn-l db pointer text-shadow-pop-parent"
              ...
              <div
                ...
                className="flex flex-row-l flex-column flex-nowrap transition-ease-in-05 static-l absolute hide-sidebar h-auto-l vh-100 w-auto-l w-50 z-5"
              >
                {tabs.map((tabObj, ind) => (
                  <NavigationTab
                    key={ind}
...
const TabContent = ({ fontSize = "f5", isLink = true, index, content }) => {
  return (
    <span
      className={`dib font-ubuntu-mono text-shadow-pop-parent ${fontSize}`}
